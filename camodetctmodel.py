...
#  CELL 4: Crop ALL images using ZoomNeXt B4
...
#  CELL 5: BACKGROUND-INVARIANT Training
#  Forces model to look at SOLDIER not background


# ──────────────────────────────────────────────
#   SET YOUR CROPPED DATASET PATH
# ──────────────────────────────────────────────
...
class BilateralFilter:
    """Smooths vegetation, KEEPS body edges sharp"""
...
class CLAHETransform:
    """Local contrast — soldier body POPS OUT from background"""
...
class MedianBlurTransform:
    """Removes leaf texture, keeps body SHAPE"""
    def __init__(self, kernel_size=7, p=0.3):
...
class LargeRandomErasing:
    """Erases LARGE background regions — forces model to focus on remaining content"""
    def __init__(self, p=0.5, scale=(0.15, 0.4)):
        self.p = p
        self.scale = scale
    ...
    def __call__(self, img):
        if random.random() < self.p:
            ...
            # Fill with random color (not black — prevents model learning "black = erased")
            fill = torch.rand(c, 1, 1)
            img[:, y:y+eh, x:x+ew] = fill
        return img


class BackgroundShuffle:
    """Randomly tints background colors — breaks background-class correlation"""
...
train_transform = transforms.Compose([
    # ── SPATIAL ──
    transforms.RandomResizedCrop(INPUT_SIZE, scale=(0.5, 1.0)),  # aggressive crop → less background
    transforms.RandomHorizontalFlip(p=0.5),
    transforms.RandomRotation(15),

    # ── BACKGROUND BREAKING ──
    BackgroundShuffle(p=0.3),    # change background colors → break bg-class correlation

    # ── BODY-FOCUSED ──
    BilateralFilter(d=9, sigma_color=75, sigma_space=75, p=0.3),
    CLAHETransform(clip_limit=2.0, tile_grid=(8, 8), p=0.3),
    MedianBlurTransform(kernel_size=7, p=0.3),
    ...
    # ── LARGE ERASING (post-tensor) → destroys background regions ──
    LargeRandomErasing(p=0.5, scale=(0.15, 0.4)),
    transforms.RandomErasing(p=0.2, scale=(0.05, 0.15)),  # small erasing too
])
...
print(f"✅ Background-invariant datasets loaded ({INPUT_SIZE}×{INPUT_SIZE}):")
...
print(f"\n   Background-breaking augmentations:")
print(f"     BackgroundShuffle(p=0.3)      → shifts bg hue randomly")
print(f"     AggressiveCrop(0.5-1.0)       → crops out more background")
print(f"     LargeRandomErasing(p=0.5)     → erases 15-40% of image")
print(f"     + BilateralFilter + CLAHE + MedianBlur (body focus)")
...
for i in range(16):
    ...
    axes[r, c].axis("off")
plt.suptitle("Background-Invariant Training (bg shuffle + large erase + crop)", fontweight='bold')
plt.tight_layout()
plt.show()

"""**SWIN MODEL**"""

#  CELL 6: Swin V2-Small — RUN BEFORE CELL 5
...
BEST_MODEL_PATH = os.path.join(SAVE_DIR, "swin_v2_small_best.pth")
...
print(f"  Phase 1: {PHASE1_EPOCHS} epochs, LR={PHASE1_LR} (head only)")
print(f"  Phase 2: {PHASE2_EPOCHS} epochs, LR={PHASE2_LR} (full fine-tune)")
print(f"  Patience: {PATIENCE}")
print(f"  Save: {BEST_MODEL_PATH}")

"""**Phase 1 Train**"""

#  CELL 8: PHASE 1 — Head only
...
#  CELL 9: PHASE 2 — Full fine-tuning
...
#   SET THESE ONCE
# ──────────────────────────────────────────────
IMAGE_PATH = "/content/Screenshot 2026-02-27 194254.png"     # ← YOUR IMAGE PATH
MODEL_PATH = "/content/drive/MyDrive/swin_checkpoints/swin_v2_small_best.pth"     # ← YOUR CHECKPOINT (swin_v2_small_best.pth)
# ──────────────────────────────────────────────
...
if has_object:
    ...
    if bbox:
        x1, y1, x2, y2 = bbox
        swin_input = original_img.crop((x1, y1, x2, y2))
        input_type = "CROPPED"
    else:
        swin_input = original_img
        input_type = "FULL"
else:
    swin_input = original_img
    input_type = "FULL (no detection)"
...
# Stage 2: Swin V2-Small
swin_input_resized = swin_input.resize((INPUT_SIZE, INPUT_SIZE), Image.LANCZOS)
img_tensor = val_transform(swin_input_resized).unsqueeze(0).to(device)
...
# GradCAM — both classes
target_layer = get_swin_target_layer(model)
swin_rgb = np.array(swin_input_resized).astype(np.float32) / 255.0

cam_pred = GradCAM(model=model, target_layers=target_layer, reshape_transform=swin_reshape_transform)
grayscale_pred = cam_pred(input_tensor=img_tensor, targets=[ClassifierOutputTarget(pred_idx)])[0, :]
overlay_pred = show_cam_on_image(swin_rgb, grayscale_pred, use_rgb=True)
cam_pred.__del__()

other_idx = 1 - pred_idx
cam_other = GradCAM(model=model, target_layers=target_layer, reshape_transform=swin_reshape_transform)
grayscale_other = cam_other(input_tensor=img_tensor, targets=[ClassifierOutputTarget(other_idx)])[0, :]
overlay_other = show_cam_on_image(swin_rgb, grayscale_other, use_rgb=True)
cam_other.__del__()
...
if has_object:
    red = np.zeros_like(img_array)
    red[:, :, 0] = 255
    alpha = np.stack([prob_map] * 3, axis=-1) * 0.6
    overlay = (img_array * (1 - alpha) + red * alpha).astype(np.uint8)
    if bbox:
        cv2.rectangle(overlay, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 255, 0), 3)
axes[1].imshow(overlay)
axes[1].set_title(f"2. ZoomNeXt\n{'✅' if has_object else '❌'} {coverage:.1f}%",
                  fontsize=11, color='green' if has_object else 'red')
axes[1].axis("off")

axes[2].imshow(swin_input_resized)
axes[2].set_title(f"3. Swin Input\n({input_type})", fontsize=11)
axes[2].axis("off")

axes[3].imshow(overlay_pred)
axes[3].set_title(f"4. GradCAM: {class_names[pred_idx]}\n({confidence:.1f}%)",
                  fontsize=11, fontweight='bold', color='darkred')
axes[3].axis("off")

axes[4].imshow(overlay_other)
axes[4].set_title(f"5. GradCAM: {class_names[other_idx]}\n({probs[other_idx]*100:.1f}%)",
                  fontsize=11, fontweight='bold', color='darkblue')
axes[4].axis("off")

axes[5].barh(class_names, probs * 100, color=['red', 'blue'])
axes[5].set_xlim(0, 100)
color = 'red' if pred_class == "camouflage" else 'blue'
axes[5].set_title(f"6. {pred_class.upper()}\n{confidence:.1f}%",
                  fontsize=13, fontweight='bold', color=color)
axes[5].set_xlabel("Confidence %")

plt.suptitle(f"🎯 {pred_class.upper()} ({confidence:.1f}%)", fontsize=15, fontweight='bold')
plt.tight_layout()
plt.show()
